...
object Funcs {
  ...
  def tail[A](ls: List[A]): List[A] = { 
    ls match { 
      case Nil => throw new IllegalArgumentException("Nil passed!!!!!!!!!")
      case _ :: x => x
    }
  }
  ...
  def drop[A](ls: List[A], n: Int): List[A] = { 
    /*
    n match { 
      case _ if (n >= ls.size) => List()
      case _ if (n == ls.size + 1) => tail(ls) 
  ...
  def reverse[A](ls: List[A]): List[A] =  
    foldLeft[A, List[A]](ls, Nil)((acc: List[A], h: A) => acc ::: List(h)) 
